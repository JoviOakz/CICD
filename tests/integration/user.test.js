...
describe('GET /:id', () => {
    ...
    it('Deve retornar o status 200 e retornar um usuário', async () => {
        User.findById.mockResolvedValue({ _id: '1', name: 'Queila', lastname: 'Lima', salary: 1234 }); // Mockando recebendo users
        const res = await request(app).get('/api/user/1');
        expect(res.status).toBe(200);
        expect(User.findById).toBeCalled();
    }),
    it('Deve retornar o status 404 e não encontrar usuário', async () => {
